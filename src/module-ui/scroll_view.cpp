...
Scroll_view::Scroll_view(std::string const& name):Widget(name){
    ...
    _up=sfml.register_callback(events::SFML_event_type::MOUSE_WHEEL_UP,
        [this](events::SFML_event_data const&){ if(!rect().contains(ui_manager().mouse_position()))return; if(_dir==Dir::HORIZONTAL)_target.x-=_speed; else _target.y-=_speed; }, listener_id());
...
void Scroll_view::on_layout(){
    ...
    // Children are laid out in on_arrange_none_children() which runs after on_layout().
}
void Scroll_view::on_arrange_none_children(sf::FloatRect const& inner){
    float content_h = 0.f;
    for(auto const& c:children()){
        sf::FloatRect s({inner.position.x-_off.x,inner.position.y-_off.y},inner.size);
        c->do_layout(s);
        // Use content_height() (overridable) so Label reports actual line height
        // rather than its fixed px() rect height, giving accurate scrollbar range.
        content_h = std::max(content_h, c->content_height());
    }
    if (content_h > 0.f) _content.y = content_h;
    // Re-clamp target now that actual content height is known.
    float my=std::max(0.f,_content.y-inner.size.y);
    _target.y=clamp(_target.y,0.f,my);
}
...
void Scroll_view::on_pre_child_render(render::Renderer& renderer) {
    renderer.push_clip(
        on_overlay() ? render::Render_layer::UI_OVERLAY : render::Render_layer::UI,
        rect()
    );
}
void Scroll_view::on_post_child_render(render::Renderer& renderer) {
    renderer.pop_clip(
        on_overlay() ? render::Render_layer::UI_OVERLAY : render::Render_layer::UI
    );
}
